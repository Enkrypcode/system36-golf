...
import type { Player, ScoringSystem, TournamentFormat } from "@/lib/scoring";
...
export function SimpleTournamentTool({ courses }: { courses: ScoringCourse[] }) {
  ...
  const winnerResult = useMemo(() => calculateTournamentWinners(rounds.map((round) => ({ id: round.id, pars: courseById.get(round.courseId ?? -1)?.pars ?? [], players: tournamentScores[round.id] ?? [] })), flights, flightLimits, { scoringSystem, tournamentFormat }), [courseById, flightLimits, flights, rounds, scoringSystem, tournamentFormat, tournamentScores]);
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      const saved: SavedTournament = { version: 1, step, name, roundCount, rounds, tournamentScores, flights, flightLimits, scoringSystem, tournamentFormat, novelties };
  ...
  }, [flightLimits, flights, hasTournamentData, name, roundCount, rounds, scoringSystem, step, storageReady, tournamentFormat, tournamentScores, novelties]);
  ...
  const loadKbpJngTestData = () => {
    ...
    setScoringSystem("system36"); setTournamentFormat("standard"); setFlights(2); setFlightLimits([12]); setExpandedFlights({});
  ...
  const loadPsgcRancamayaData = () => {
    ...
    setScoringSystem("handicap"); setTournamentFormat("psgc"); setFlights(3); setFlightLimits([14, 19]); setExpandedFlights({});
    ...
    <section className="simple-content">
      {step === 1 && <section className="setup-step"><div className="step-intro"><p>STEP 1</p><h1>Tournament setup</h1><span>Choose the tournament and its rounds. Course pars load automatically.</span></div>{fixtureLoadError && <p className="validation-error">{fixtureLoadError}</p>}
        ...
        <div className="setup-inline-fields">
          ...
          {scoringSystem === "handicap" && <label className="setup-format">Tournament format<select value={tournamentFormat} onChange={(event) => setTournamentFormat(event.target.value as TournamentFormat)}><option value="standard">Standard</option><option value="psgc">PSGC</option></select></label>}
        </div>
      ...
      {step === 3 && <section className="awards-step">
        ...
        <aside className="tie-break-rules"><b>Tie-break Rules</b><span><strong>Nett</strong> {scoringSystem === "handicap" ? "Lower Handicap" : "Lower HCP"} → CB9 → CB6 → CB3 → CB1</span><span><strong>Gross</strong> CB9 → CB6 → CB3 → CB1</span><small>Applied only when the primary result is tied.</small></aside>
