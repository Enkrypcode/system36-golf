...
export function SimpleTournamentTool({ courses }: { courses: ScoringCourse[] }) {
  ...
  const [awardMode, setAwardMode] = useState<AwardMode>("gross-nett");
  ...
  const winnerResult = useMemo(() => calculateTournamentWinners(rounds.map((round) => ({ id: round.id, pars: courseById.get(round.courseId ?? -1)?.pars ?? [], players: tournamentScores[round.id] ?? [] })), flights, flightLimits, { scoringSystem, tournamentFormat, nettTieBreakMethod, awardMode }), [courseById, flightLimits, flights, rounds, scoringSystem, tournamentFormat, nettTieBreakMethod, awardMode, tournamentScores]);
  ...
  const nettOnlyAwards = scoringSystem === "handicap" && !splitNineMode && awardMode === "nett-only";
  ...
  const awardSections: Array<[string, string[]]> = [["Overall", nettOnlyAwards ? ["BN 1", "BN 2", "BN 3"] : ["BGO", "BNO"]], ...flightAwardSections];
  ...
  useEffect(() => {
    restoreTimerRef.current = window.setTimeout(() => {
      ...
      if (saved) {
        ...
        setAwardMode(saved.awardMode ?? "gross-nett");
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      const saved: SavedTournament = { version: 1, step, name, roundCount, rounds, tournamentScores, flights, flightLimits, scoringSystem, tournamentFormat, awardMode, nettTieBreakMethod, system36NettTieBreakMethod, handicapNettTieBreakMethod, novelties, jackpot };
  ...
  }, [flightLimits, flights, hasTournamentData, name, roundCount, rounds, scoringSystem, step, storageReady, tournamentFormat, awardMode, nettTieBreakMethod, system36NettTieBreakMethod, handicapNettTieBreakMethod, tournamentScores, novelties, jackpot]);
  ...
  const loadKbpJngTestData = () => {
    ...
    setScoringSystem("system36"); setTournamentFormat("standard"); setAwardMode("gross-nett"); setSystem36NettTieBreakMethod("lower-handicap"); setHandicapNettTieBreakMethod("countback"); setFlights(2); setFlightLimits([12]); setExpandedFlights({});
  ...
  const loadPsgcRancamayaData = () => {
    ...
    setScoringSystem("handicap"); setTournamentFormat("psgc"); setAwardMode("gross-nett"); setHandicapNettTieBreakMethod("lower-handicap"); setFlights(3); setFlightLimits([14, 19]); setExpandedFlights({});
  ...
  const loadGobarHendraSplitNineData = () => {
    ...
    setName(fixture.name); setRoundCount(1); setRounds(fixture.rounds); setTournamentScores(fixture.scores); setNovelties([]); setJackpot(defaultJackpotSettings());
    setScoringSystem(fixture.scoringSystem); setTournamentFormat(fixture.tournamentFormat); setAwardMode("gross-nett"); setHandicapNettTieBreakMethod("countback"); setFlights(2); setFlightLimits([12]); setExpandedFlights({});
    setFixtureLoadError(""); setFixtureLoadNotice(""); finishFixtureLoad();
  };
