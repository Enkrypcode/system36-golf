...
export function SimpleTournamentTool({ courses }: { courses: ScoringCourse[] }) {
  ...
  const splitNineExportLeaders = useMemo(() => {
    const players = winnerResult.splitNine?.leaderboard ?? [];
    const rank = (items: typeof players, metric: "frontNett" | "backNett") => {
      const sorted = [...items].sort((left, right) => left[metric] - right[metric] || left.name.localeCompare(right.name));
      return sorted.map((player, index) => ({ player, rank: index === 0 || player[metric] !== sorted[index - 1][metric] ? index + 1 : sorted.findIndex((item) => item[metric] === player[metric]) + 1 }));
    };
    const firstChampion = winnerResult.splitNine?.firstChampion;
    const secondChampion = winnerResult.splitNine?.secondChampion;
    const front = rank(players, "frontNett");
    const back = rank(players.filter((player) => player.key !== firstChampion?.key), "backNett");
    return {
      front,
      back,
      firstChampionId: firstChampion?.key,
      secondChampionId: secondChampion?.key,
      firstRunnerUpId: firstChampion ? front.find((entry) => entry.player.key !== firstChampion.key)?.player.key : undefined,
      secondRunnerUpId: secondChampion ? back.find((entry) => entry.player.key !== secondChampion.key)?.player.key : undefined,
    };
  ...
  return <main className="simple-tool">
    ...
    <section className="simple-content">
      ...
      {step === 3 && <section className="awards-step">
        ...
        {splitNineMode ? <section className="split-nine-results"><div className="split-nine-heading"><p>Dedicated Handicap competition</p><h2>Split 9-Hole Results</h2><span>½ Handicap is applied separately to each nine. Tied results remain manual decisions.</span></div><div className="split-nine-grid"><section><h3>1st Nine Champion</h3>{winnerResult.splitNine?.firstChampion ? <div className="split-nine-champion"><strong>{winnerResult.splitNine.firstChampion.name}</strong><span>Front Gross {winnerResult.splitNine.firstChampion.frontGross}</span><span>½ HCP {winnerResult.splitNine.firstChampion.halfHandicap}</span><b>1st Nine Nett {winnerResult.splitNine.firstChampion.frontNett}</b></div> : <p className="split-nine-tied">{winnerResult.splitNine?.firstTied?.length ? "Tied — Manual Decision" : "No eligible player"}</p>}<div className="split-nine-table-wrap"><table><thead><tr><th>Rank</th><th>Player</th><th>Gross</th><th>½ HCP</th><th>Nett</th><th>Status</th></tr></thead><tbody>{[...(winnerResult.splitNine?.leaderboard ?? [])].sort((left, right) => left.frontRank - right.frontRank).map((player) => <tr key={player.key}><td>{player.frontRank}</td><td>{player.name}</td><td>{player.frontGross}</td><td>{player.halfHandicap}</td><td>{player.frontNett}</td><td>{winnerResult.splitNine?.firstChampion?.key === player.key ? "Champion" : winnerResult.splitNine?.firstTied?.some((tied) => tied.key === player.key) ? "Tied — Manual Decision" : "Eligible"}</td></tr>)}</tbody></table></div></section><section><h3>2nd Nine Champion</h3>{winnerResult.splitNine?.secondChampion ? <div className="split-nine-champion"><strong>{winnerResult.splitNine.secondChampion.name}</strong><span>Back Gross {winnerResult.splitNine.secondChampion.backGross}</span><span>½ HCP {winnerResult.splitNine.secondChampion.halfHandicap}</span><b>2nd Nine Nett {winnerResult.splitNine.secondChampion.backNett}</b></div> : <p className="split-nine-tied">{winnerResult.splitNine?.secondTied?.length ? "Tied — Manual Decision" : "No eligible player"}</p>}<div className="split-nine-table-wrap"><table><thead><tr><th>Rank</th><th>Player</th><th>Gross</th><th>½ HCP</th><th>Nett</th><th>Status</th></tr></thead><tbody>{[...(winnerResult.splitNine?.leaderboard ?? [])].sort((left, right) => left.backRank - right.backRank).map((player) => <tr key={player.key}><td>{player.backRank}</td><td>{player.name}</td><td>{player.backGross}</td><td>{player.halfHandicap}</td><td>{player.backNett}</td><td>{winnerResult.splitNine?.firstChampion?.key === player.key ? "Already won 1st Nine" : winnerResult.splitNine?.secondChampion?.key === player.key ? "Champion" : winnerResult.splitNine?.secondTied?.some((tied) => tied.key === player.key) ? "Tied — Manual Decision" : "Eligible"}</td></tr>)}</tbody></table></div></section></div></section> : <>
    ...
    <div className="winner-export-canvas" aria-hidden="true"><section className="winner-export-sheet" ref={exportSheetRef}><header><p>Golf Scoring System</p><h1>{name || "Golf Tournament"}</h1><span>{rounds.length} {rounds.length === 1 ? "Round" : "Rounds"} · {roundSummary.join("  |  ")}</span></header>{!splitNineMode && <div className="winner-export-summary"><div><b>{winnerResult.eligible.length}</b><span>Eligible players</span></div><div><b>{winnerResult.notEligible}</b><span>Not eligible</span></div><div><b>{psgcMode ? 4 : flights}</b><span>{psgcMode ? "Award categories" : flights === 1 ? "Flight" : "Flights"}</span></div></div>}{!splitNineMode && <p className="winner-export-ranges">{psgcMode ? "PSGC explicit categories: Flight A · Flight B · Flight C · Super Senior" : Array.from({ length: flights }, (_, index) => `Flight ${String.fromCharCode(65 + index)}: ${flightRange(index)}`).join(" · ")}</p>}{splitNineMode ? <section className="split-nine-export"><h2>Split 9-Hole Results</h2><div className="split-nine-export-grid"><section><h3>1st Nine Leaderboard</h3><table><thead><tr><th>Rank</th><th>Player Name</th><th>Gross</th><th>½ Handicap</th><th>Nett</th></tr></thead><tbody>{splitNineExportLeaders.front.map((entry) => <tr className={entry.player.key === splitNineExportLeaders.firstChampionId ? "split-nine-export-winner" : entry.player.key === splitNineExportLeaders.firstRunnerUpId ? "split-nine-export-runner-up" : ""} key={entry.player.key}><td>{entry.rank}</td><td><strong>{entry.player.name}</strong>{entry.player.key === splitNineExportLeaders.firstChampionId && <span>Winner</span>}{entry.player.key === splitNineExportLeaders.firstRunnerUpId && <span>Runner-up</span>}</td><td>{entry.player.frontGross}</td><td>{entry.player.halfHandicap}</td><td>{entry.player.frontNett}</td></tr>)}</tbody></table></section><section><h3>2nd Nine Leaderboard</h3><table><thead><tr><th>Rank</th><th>Player Name</th><th>Gross</th><th>½ Handicap</th><th>Nett</th></tr></thead><tbody>{splitNineExportLeaders.back.map((entry) => <tr className={entry.player.key === splitNineExportLeaders.secondChampionId ? "split-nine-export-winner" : entry.player.key === splitNineExportLeaders.secondRunnerUpId ? "split-nine-export-runner-up" : ""} key={entry.player.key}><td>{entry.rank}</td><td><strong>{entry.player.name}</strong>{entry.player.key === splitNineExportLeaders.secondChampionId && <span>Winner</span>}{entry.player.key === splitNineExportLeaders.secondRunnerUpId && <span>Runner-up</span>}</td><td>{entry.player.backGross}</td><td>{entry.player.halfHandicap}</td><td>{entry.player.backNett}</td></tr>)}</tbody></table></section></div></section> : <><div className="winner-export-awards">{awardSections.map(([title, codes]) => <article key={title}><h2>{title}</h2>{codes.map((code) => { const award = winnerResult.awards.find((item) => item.code === code); const metric = award ? awardMetric(award) : ""; return <div className="winner-export-award" key={code}><b>{code}</b>{award?.winner ? <span><strong>{award.winner.name}</strong><small>{metric} {metric === "Gross" ? award.winner.aggregateGross : award.winner.aggregateNett}</small><em>{(metric === "Gross" ? award.winner.roundGross : award.winner.roundNett).map((value, index) => `R${index + 1} ${value}`).join(" + ")}</em>{award.countbackStage && <i>Tie-break: {tieBreakStage(award)}{tieBreakOpponents(award) && <><br />vs {tieBreakOpponents(award)}</>}</i>}</span> : <span>{award?.tied ? "TIE — Manual Decision" : "No player awarded"}</span>}</div>; })}</article>)}</div></>}{completedNovelties.length > 0 && <section className="winner-export-novelties"><h2>Novelties</h2>{completedNovelties.map((novelty) => <p key={novelty.id}>{noveltyText(novelty, noveltyPlayerNames.get(novelty.playerId)!)}</p>)}</section>}<footer>Generated by Golf Scoring System · {exportTimestamp}</footer></section></div>
  </main>;
}
