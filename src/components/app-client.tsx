...
export function SimpleTournamentTool({ courses }: { courses: ScoringCourse[] }) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      const saved: SavedTournament = { version: 1, step, name, roundCount, rounds, tournamentScores, flights, flightLimits, scoringSystem, tournamentFormat, nettTieBreakMethod, system36NettTieBreakMethod, handicapNettTieBreakMethod, novelties, jackpot };
  ...
  }, [flightLimits, flights, hasTournamentData, name, roundCount, rounds, scoringSystem, step, storageReady, tournamentFormat, nettTieBreakMethod, system36NettTieBreakMethod, handicapNettTieBreakMethod, tournamentScores, novelties, jackpot]);
  ...
  const loadGobarHendraSplitNineData = () => {
    ...
    setName(fixture.name); setRoundCount(1); setRounds(fixture.rounds); setTournamentScores(fixture.scores); setNovelties([]); setJackpot(defaultJackpotSettings());
    setScoringSystem(fixture.scoringSystem); setTournamentFormat(fixture.tournamentFormat); setHandicapNettTieBreakMethod("countback"); setFlights(2); setFlightLimits([12]); setExpandedFlights({});
    setFixtureLoadError(""); setFixtureLoadNotice(""); finishFixtureLoad();
  };
    ...
    <section className="simple-content">
      {step === 1 && <section className="setup-step"><div className="step-intro"><p>STEP 1</p><h1>Tournament setup</h1><span>Choose the tournament and its rounds. Course pars load automatically.</span><p className="device-recommendation">Best viewed on tablet or desktop. Mobile is supported with horizontal scrolling.</p></div>{fixtureLoadError && <p className="validation-error">{fixtureLoadError}</p>}
        ...
        <section className="jackpot-setup">
          ...
          {jackpot.enabled && <div className="blind-hole-setup"><div><b>Blind Hole Setup</b><small>Choose 2 Front and 2 Back holes. Blind-hole PAR is used only for Jackpot results.</small></div>{jackpot.locked ? <p className="jackpot-locked">🔒 4 Blind Holes Locked</p> : <><div className="blind-hole-selects"><label>Front 1–9{[0, 1].map((index) => <select key={index} value={jackpot.blindHoles.filter((hole) => hole <= 9)[index] ?? ""} onChange={(event) => updateJackpotBlindHole("front", index, event.target.value)}><option value="">Select H{index + 1}</option>{Array.from({ length: 9 }, (_, value) => value + 1).map((hole) => <option key={hole} value={hole} disabled={jackpot.blindHoles.includes(hole) && jackpot.blindHoles.filter((item) => item <= 9)[index] !== hole}>H{hole}</option>)}</select>)}</label><label>Back 10–18{[0, 1].map((index) => <select key={index} value={jackpot.blindHoles.filter((hole) => hole >= 10)[index] ?? ""} onChange={(event) => updateJackpotBlindHole("back", index, event.target.value)}><option value="">Select H{index + 10}</option>{Array.from({ length: 9 }, (_, value) => value + 10).map((hole) => <option key={hole} value={hole} disabled={jackpot.blindHoles.includes(hole) && jackpot.blindHoles.filter((item) => item >= 10)[index] !== hole}>H{hole}</option>)}</select>)}</label></div><button className="secondary-button" type="button" disabled={!isValidBlindHoles(jackpot.blindHoles)} onClick={lockBlindHoles}>Lock Blind Holes</button></>}</div>}
        </section>
