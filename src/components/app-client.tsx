...
export function SimpleTournamentTool({ courses }: { courses: ScoringCourse[] }) {
  ...
  const winnerResult = useMemo(() => calculateTournamentWinners(rounds.map((round) => ({ id: round.id, pars: courseById.get(round.courseId ?? -1)?.pars ?? [], players: tournamentScores[round.id] ?? [] })), flights, flightLimits, { scoringSystem, tournamentFormat, nettTieBreakMethod, awardMode, overallAwards }), [courseById, flightLimits, flights, rounds, scoringSystem, tournamentFormat, nettTieBreakMethod, awardMode, overallAwards, tournamentScores]);
  ...
  const jackpotFrontResults = useMemo(() => jackpot.enabled ? calculateJackpotResults(jackpotPlayers, jackpotPars, jackpot.blindHoles, "front") : [], [jackpot.enabled, jackpot.blindHoles, jackpotPars, jackpotPlayers]);
  const jackpotBackResults = useMemo(() => jackpot.enabled ? calculateJackpotResults(jackpotPlayers, jackpotPars, jackpot.blindHoles, "back") : [], [jackpot.enabled, jackpot.blindHoles, jackpotPars, jackpotPlayers]);
  ...
  const standaloneNettAwards = !psgcMode && !splitNineMode && nettOnlyAwards && !overallAwards && flights === 0;
  ...
  const awardSections: Array<[string, string[]]> = [...(overallAwards || psgcMode ? [["Overall", nettOnlyAwards ? ["BN 1", "BN 2", "BN 3"] : ["BGO", "BNO"]] as [string, string[]]] : []), ...(standaloneNettAwards ? [["", ["BN 1", "BN 2", "BN 3"]] as [string, string[]]] : []), ...flightAwardSections];
  ...
  useEffect(() => {
    restoreTimerRef.current = window.setTimeout(() => {
      ...
      if (saved) {
        ...
        setOverallAwards(saved.overallAwards ?? true); setFlights(saved.flights);
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      const saved: SavedTournament = { version: 1, step, name, roundCount, rounds, tournamentScores, flights, flightLimits, overallAwards, scoringSystem, tournamentFormat, awardMode, system36AwardMode, handicapAwardMode, nettTieBreakMethod, system36NettTieBreakMethod, handicapNettTieBreakMethod, novelties, jackpot };
  ...
  }, [flightLimits, flights, overallAwards, hasTournamentData, name, roundCount, rounds, scoringSystem, step, storageReady, tournamentFormat, awardMode, system36AwardMode, handicapAwardMode, nettTieBreakMethod, system36NettTieBreakMethod, handicapNettTieBreakMethod, tournamentScores, novelties, jackpot]);
  ...
  const startNewTournament = () => {
    ...
    setOverallAwards(true); setFlights(2);
    setFlightLimits([12]);
    setExpandedFlights({});
  ...
  const loadKbpJngTestData = () => {
    ...
    setScoringSystem("system36"); setTournamentFormat("standard"); setSystem36AwardMode("gross-nett"); setHandicapAwardMode("gross-nett"); setSystem36NettTieBreakMethod("lower-handicap"); setHandicapNettTieBreakMethod("countback"); setOverallAwards(true); setFlights(2); setFlightLimits([12]); setExpandedFlights({});
  ...
  const loadPsgcRancamayaData = () => {
    ...
    setScoringSystem("handicap"); setTournamentFormat("psgc"); setSystem36AwardMode("gross-nett"); setHandicapAwardMode("gross-nett"); setHandicapNettTieBreakMethod("lower-handicap"); setFlights(3); setFlightLimits([14, 19]); setExpandedFlights({});
  ...
  const loadGobarHendraSplitNineData = () => {
    ...
    setName(fixture.name); setRoundCount(1); setRounds(fixture.rounds); setTournamentScores(fixture.scores); setNovelties([]); setJackpot(defaultJackpotSettings());
    setScoringSystem(fixture.scoringSystem); setTournamentFormat(fixture.tournamentFormat); setSystem36AwardMode("gross-nett"); setHandicapAwardMode("gross-nett"); setHandicapNettTieBreakMethod("countback"); setOverallAwards(true); setFlights(2); setFlightLimits([12]); setExpandedFlights({});
    setFixtureLoadError(""); setFixtureLoadNotice(""); finishFixtureLoad();
  };
  ...
  return <main className="simple-tool">
    ...
    <section className="simple-content">
      {step === 1 && <section className="setup-step"><div className="step-intro"><p>STEP 1</p><h1>Tournament setup</h1><span>Choose the tournament and its rounds. Course pars load automatically.</span><p className="device-recommendation">Best viewed on tablet or desktop. Mobile is supported with horizontal scrolling.</p></div>{fixtureLoadError && <p className="validation-error">{fixtureLoadError}</p>}
        ...
        {jackpot.enabled && <section className="jackpot-setup"><div className="blind-hole-setup"><div><b>Blind Hole Setup</b><small>Choose 2 Front and 2 Back holes. Blind-hole PAR is used only for Jackpot results.</small></div>{jackpot.locked ? <p className="jackpot-locked">🔒 4 Blind Holes Locked</p> : <><div className="blind-hole-selects"><label>Front 1–9{[0, 1].map((index) => <select key={index} value={jackpot.blindHoles.filter((hole) => hole <= 9)[index] ?? ""} onChange={(event) => updateJackpotBlindHole("front", index, event.target.value)}><option value="">Select H{index + 1}</option>{Array.from({ length: 9 }, (_, value) => value + 1).map((hole) => <option key={hole} value={hole} disabled={jackpot.blindHoles.includes(hole) && jackpot.blindHoles.filter((item) => item <= 9)[index] !== hole}>H{hole}</option>)}</select>)}</label><label>Back 10–18{[0, 1].map((index) => <select key={index} value={jackpot.blindHoles.filter((hole) => hole >= 10)[index] ?? ""} onChange={(event) => updateJackpotBlindHole("back", index, event.target.value)}><option value="">Select H{index + 10}</option>{Array.from({ length: 9 }, (_, value) => value + 10).map((hole) => <option key={hole} value={hole} disabled={jackpot.blindHoles.includes(hole) && jackpot.blindHoles.filter((item) => item >= 10)[index] !== hole}>H{hole}</option>)}</select>)}</label></div><button className="secondary-button" type="button" disabled={!isValidBlindHoles(jackpot.blindHoles)} onClick={lockBlindHoles}>Lock Blind Holes</button></>}</div></section>}
      ...
      {step === 3 && <section className="awards-step">
        ...
        </>}{jackpot.enabled && <section className="jackpot-results"><div className="jackpot-results-header"><div><p>Additional side competition</p><h2>Jackpot Results</h2><span>{jackpotRound ? "Round " + (rounds.findIndex((round) => round.id === jackpotRound.id) + 1) : ""}</span></div>{jackpot.revealed ? <p className="jackpot-revealed-holes">Blind Holes: {jackpot.blindHoles.map((hole) => "H" + hole).join(" · ")}</p> : <button className="secondary-button" type="button" disabled={!jackpot.locked} onClick={() => setShowJackpotRevealConfirm(true)}>{jackpot.locked ? "Reveal Blind Holes" : "Blind Holes Not Locked"}</button>}</div><div className="jackpot-result-grid">{([["TARGET 1–9", jackpotFrontResults], ["TARGET 10–18", jackpotBackResults]] as const).map(([title, results]) => <section key={title}><h3>{title}</h3><div className="jackpot-table-wrap"><table><thead><tr><th>No</th><th>Player Name</th><th>Target</th><th>Final</th><th>Delta</th><th>Result</th></tr></thead><tbody>{results.map((entry, index) => <tr key={entry.playerId}><td>{entry.rank ?? index + 1}</td><td>{entry.name || "Unnamed player"}</td><td>{entry.target ?? "—"}</td><td>{entry.final ?? "—"}</td><td>{entry.result === "DIS" ? "DIS" : entry.delta ?? "—"}</td><td>{entry.result === "DIS" ? "DIS" : entry.result === "ELIGIBLE" ? entry.rank === 1 ? "Leading" : "Eligible" : "Incomplete"}</td></tr>)}</tbody></table></div></section>)}</div></section>}
