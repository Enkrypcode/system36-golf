  ...
  return <main className="simple-tool">
    ...
    <section className="simple-content">
      ...
      {step === 3 && <section className="awards-step">
        ...
        {splitNineMode ? <section className="split-nine-results"><div className="split-nine-heading"><p>Dedicated Handicap competition</p><h2>Split 9-Hole Results</h2><span>½ Handicap is applied separately to each nine. Tied results remain manual decisions.</span></div><div className="split-nine-grid"><section><h3>1st Nine Champion</h3>{winnerResult.splitNine?.firstChampion ? <div className="split-nine-champion"><strong>{winnerResult.splitNine.firstChampion.name}</strong><span>Front Gross {winnerResult.splitNine.firstChampion.frontGross}</span><span>½ HCP {winnerResult.splitNine.firstChampion.halfHandicap}</span><b>1st Nine Nett {winnerResult.splitNine.firstChampion.frontNett}</b></div> : <p className="split-nine-tied">{winnerResult.splitNine?.firstTied?.length ? "Tied — Manual Decision" : "No eligible player"}</p>}<div className="split-nine-table-wrap"><table><thead><tr><th>Rank</th><th>Player</th><th>Gross</th><th>½ HCP</th><th>Nett</th><th>Award/Status</th></tr></thead><tbody>{splitNineLeaderboards.appFront.map((entry) => <tr key={entry.player.key}><td>{entry.rank}</td><td>{entry.player.name}</td><td>{entry.player.frontGross}</td><td>{entry.player.halfHandicap}</td><td>{entry.player.frontNett}</td><td>{entry.award && <strong className="split-nine-award-label">{entry.award}</strong>}{entry.note && <small className="split-nine-award-note">{entry.note}</small>}</td></tr>)}</tbody></table></div></section><section><h3>2nd Nine Champion</h3>{winnerResult.splitNine?.secondChampion ? <div className="split-nine-champion"><strong>{winnerResult.splitNine.secondChampion.name}</strong><span>Back Gross {winnerResult.splitNine.secondChampion.backGross}</span><span>½ HCP {winnerResult.splitNine.secondChampion.halfHandicap}</span><b>2nd Nine Nett {winnerResult.splitNine.secondChampion.backNett}</b></div> : <p className="split-nine-tied">{winnerResult.splitNine?.secondTied?.length ? "Tied — Manual Decision" : "No eligible player"}</p>}<div className="split-nine-table-wrap"><table><thead><tr><th>Rank</th><th>Player</th><th>Gross</th><th>½ HCP</th><th>Nett</th><th>Award/Status</th></tr></thead><tbody>{splitNineLeaderboards.appBack.map((entry) => <tr key={entry.player.key}><td>{entry.rank}</td><td>{entry.player.name}</td><td>{entry.player.backGross}</td><td>{entry.player.halfHandicap}</td><td>{entry.player.backNett}</td><td>{entry.award && <strong className="split-nine-award-label">{entry.award}</strong>}{entry.note && <small className="split-nine-award-note">{entry.note}</small>}</td></tr>)}</tbody></table></div></section></div></section> : <>
