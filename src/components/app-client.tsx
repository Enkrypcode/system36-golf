...
const tieBreakStage = (award: Award) => award.countbackStage === "HCP" ? "Lower HCP" : award.countbackStage === "HANDICAP" ? "Lower Handicap" : award.countbackStage;
...
export function SimpleTournamentTool({ courses }: { courses: ScoringCourse[] }) {
  ...
  const [nettTieBreakMethod, setNettTieBreakMethod] = useState<NettTieBreakMethod>("countback");
  ...
  useEffect(() => {
    restoreTimerRef.current = window.setTimeout(() => {
      ...
      if (saved) {
        ...
        setNettTieBreakMethod(saved.nettTieBreakMethod ?? defaultNettTieBreakMethod(saved.scoringSystem ?? "system36", saved.tournamentFormat ?? "standard"));
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      const saved: SavedTournament = { version: 1, step, name, roundCount, rounds, tournamentScores, flights, flightLimits, scoringSystem, tournamentFormat, nettTieBreakMethod, novelties };
  ...
  }, [flightLimits, flights, hasTournamentData, name, roundCount, rounds, scoringSystem, step, storageReady, tournamentFormat, nettTieBreakMethod, tournamentScores, novelties]);
  ...
  const loadKbpJngTestData = () => {
    ...
    setScoringSystem("system36"); setTournamentFormat("standard"); setNettTieBreakMethod("countback"); setFlights(2); setFlightLimits([12]); setExpandedFlights({});
  ...
  const loadPsgcRancamayaData = () => {
    ...
    setScoringSystem("handicap"); setTournamentFormat("psgc"); setNettTieBreakMethod("lower-handicap"); setFlights(3); setFlightLimits([14, 19]); setExpandedFlights({});
    ...
    <section className="simple-content">
      {step === 1 && <section className="setup-step"><div className="step-intro"><p>STEP 1</p><h1>Tournament setup</h1><span>Choose the tournament and its rounds. Course pars load automatically.</span></div>{fixtureLoadError && <p className="validation-error">{fixtureLoadError}</p>}
        ...
        <div className="setup-inline-fields">
          <div className="setup-scoring-field"><span>Scoring system</span><div className="scoring-system-toggle" role="group" aria-label="Scoring system"><button type="button" className={scoringSystem === "system36" ? "active" : ""} aria-pressed={scoringSystem === "system36"} onClick={() => { setScoringSystem("system36"); setTournamentFormat("standard"); setNettTieBreakMethod("countback"); }}>System 36</button><button type="button" className={scoringSystem === "handicap" ? "active" : ""} aria-pressed={scoringSystem === "handicap"} onClick={() => setScoringSystem("handicap")}>Handicap</button></div>{scoringSystem === "handicap" && <small>Manual tournament / playing handicap</small>}</div>
          ...
          {scoringSystem === "handicap" && <><label className="setup-format">Tournament format<select value={tournamentFormat} onChange={(event) => { const format = event.target.value as TournamentFormat; setTournamentFormat(format); setNettTieBreakMethod(defaultNettTieBreakMethod("handicap", format)); }}><option value="standard">Standard</option><option value="psgc">PSGC</option></select></label><div className="setup-scoring-field nett-tie-break-field"><span>Nett tie-break method</span><div className="scoring-system-toggle" role="group" aria-label="Nett tie-break method"><button type="button" className={nettTieBreakMethod === "lower-handicap" ? "active" : ""} aria-pressed={nettTieBreakMethod === "lower-handicap"} onClick={() => setNettTieBreakMethod("lower-handicap")}>Lower Handicap</button><button type="button" className={nettTieBreakMethod === "countback" ? "active" : ""} aria-pressed={nettTieBreakMethod === "countback"} onClick={() => setNettTieBreakMethod("countback")}>Countback</button></div></div></>}
        </div>
      ...
      {step === 3 && <section className="awards-step">
        ...
        <aside className="tie-break-rules"><b>Tie-break Rules</b><span><strong>Nett</strong> {scoringSystem === "handicap" ? nettTieBreakMethod === "lower-handicap" ? "Lower Handicap → CB9 → CB6 → CB3 → CB1" : "CB9 → CB6 → CB3 → CB1" : "Lower HCP → CB9 → CB6 → CB3 → CB1"}</span><span><strong>Gross</strong> CB9 → CB6 → CB3 → CB1</span><small>Applied only when the primary result is tied.</small></aside>
