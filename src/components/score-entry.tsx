...
import { scoreEntryGrossCsv, scoreEntryGrossFilename, scoreEntryResultsCsv, scoreEntryResultsFilename, scoreEntryScoresCsv, scoreEntryScoresFilename } from "@/lib/score-entry-export";
import { applyScoreImport, previewScoreImportFile, type ScoreImportMode, type ScoreImportPreview } from "@/lib/score-import";
...
import type { JackpotMode } from "@/lib/jackpot";
...
type Props = {
  ...
  scoringSystem: ScoringSystem; tournamentFormat: TournamentFormat; jackpotEnabled: boolean; jackpotMode: JackpotMode; jackpotBlindHoles: number[]; jackpotRevealed: boolean;
...
export function ScoreEntry({ tournamentName, rounds, courses, savedScores, scoringSystem, tournamentFormat, jackpotEnabled, jackpotMode, jackpotBlindHoles, jackpotRevealed, onScoresChange, onHandicapChange, onBack, onContinue }: Props) {
  const [roundId, setRoundId] = useState(rounds[0].id);
  const gridInputRefs = useRef<Record<string, HTMLInputElement | null>>({});
  const manualJackpotInputRefs = useRef<Record<string, HTMLInputElement | null>>({});
  ...
  const [scoreImportMode, setScoreImportMode] = useState<ScoreImportMode>("scores-only");
  ...
  const blindHoleJackpot = jackpotEnabled && jackpotMode === "blind-hole";
  const manualJackpot = jackpotEnabled && jackpotMode === "manual";
  ...
  const awardCategoryColumn = psgcMode ? (handicapMode ? 2 : 1) : null; const targetStart = 1 + (handicapMode ? 1 : 0) + (psgcMode ? 1 : 0); const pairingColumn = targetStart + (blindHoleJackpot ? 2 : 0); const holeStart = pairingColumn + 1; const lastColumn = holeStart + 17;
  ...
  const updateManualJackpotValue = (playerId: string, field: "jackpotManualFirstNine" | "jackpotManualSecondNine" | "jackpotManualNett", raw: string) => { if (raw !== "" && (!/^\d{1,3}(\.\d{0,2})?$/.test(raw) || Number(raw) < 0)) return; const value = raw === "" ? undefined : Number(raw); setScoresByRound((current) => ({ ...current, [activeRound.id]: current[activeRound.id].map((player) => player.id === playerId ? { ...player, [field]: value } : player) })); };
  const manualJackpotFields = ["jackpotManualFirstNine", "jackpotManualSecondNine", "jackpotManualNett", "jackpotTargetFront", "jackpotTargetBack"] as const;
  const moveManualJackpotFocus = (event: React.KeyboardEvent<HTMLInputElement>, playerIndex: number, fieldIndex: number) => {
    let nextPlayer = playerIndex; let nextField = fieldIndex;
    if (event.key === "ArrowLeft") nextField -= 1;
    else if (event.key === "ArrowRight") nextField += 1;
    else if (event.key === "ArrowUp") nextPlayer -= 1;
    else if (event.key === "ArrowDown" || event.key === "Enter") nextPlayer += 1;
    else if (event.key === "Tab") { nextField += event.shiftKey ? -1 : 1; }
    else return;
    if (nextField < 0) { nextPlayer -= 1; nextField = manualJackpotFields.length - 1; }
    if (nextField >= manualJackpotFields.length) { nextPlayer += 1; nextField = 0; }
    if (nextPlayer < 0 || nextPlayer >= players.length) return;
    event.preventDefault();
    const next = manualJackpotInputRefs.current[`${players[nextPlayer].id}:${manualJackpotFields[nextField]}`];
  ...
  const activeRoundNumber = () => rounds.findIndex((round) => round.id === activeRound.id) + 1;
  const exportScoresCsv = () => downloadCsv(
    scoreEntryScoresCsv(players),
    scoreEntryScoresFilename(tournamentName, activeRoundNumber()),
  );
  const exportResultsCsv = () => downloadCsv(
    scoreEntryResultsCsv(players, course.pars!, { scoringSystem, tournamentFormat }),
    scoreEntryResultsFilename(tournamentName, activeRoundNumber()),
  );
  const exportGrossCsv = () => downloadCsv(
    scoreEntryGrossCsv(players, course.pars!),
    scoreEntryGrossFilename(tournamentName, activeRoundNumber()),
  );
  const chooseScoreImportFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try { setScoreImportPreview(await previewScoreImportFile(file, players, scoreImportMode)); }
  ...
  const confirmScoreImport = () => {
    ...
    setScoresByRound((current) => ({ ...current, [activeRound.id]: applyScoreImport(current[activeRound.id] ?? [], scoreImportPreview.entries, { createPlayerId: (_entry, index) => `import-player-${Date.now()}-${index}-${Math.random().toString(36).slice(2, 8)}` }) }));
    closeScoreImport();
  };
  ...
  useEffect(() => {
    ...
    const closeOnOutside = (event: PointerEvent) => { if (!exportMenuRef.current?.contains(event.target as Node)) setExportMenuOpen(false); };
